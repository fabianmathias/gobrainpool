...
// nonces. It is instantiated ex novo for each signature and produces
// successive candidates k in [1, N-1] via next(); the spec's retry rule
// (§3.2 step h for RFC 6979; equivalent 10.1.2.5 for plain HMAC-DRBG)
// is applied on out-of-range emissions and between calls so that a
// caller who rejects the previous k transparently advances the state.
//
...
//     function SHA-512 (matching stdlib crypto/ecdsa). See newHedgedGen.
type hmacDRBG struct {
	newHash func() hash.Hash
	K, V    []byte
	qbytes  int
	nBE     []byte // N as a big-endian byte slice of length qbytes
	primed  bool   // true once next() has emitted at least one value
}
...
// are folded in as a block-aligned personalization string. The digest
// hash is SHA-512 irrespective of curve — matching stdlib crypto/ecdsa,
// which accepts the resulting minor deviation from the draft (it has no
// security impact; see stdlib signFIPS comment).
func newHedgedGen(c *Curve, xBytes, hashMsg, Z []byte) *hmacDRBG {
	h1 := hashToScalarBytes(hashMsg, c)
	return newBlockAlignedHMACDRBG(sha512.New, Z, nil, [][]byte{xBytes, h1}, c.byteSize, c.nBE)
}

func rfc6979Hash(c *Curve) (func() hash.Hash, error) {
	switch c {
	case bp256r1:
...
func (g *hmacDRBG) generate(n int) []byte {
	if g.primed {
		g.K = g.hmac(g.V, []byte{0x00})
		g.V = g.hmac(g.V)
	}
